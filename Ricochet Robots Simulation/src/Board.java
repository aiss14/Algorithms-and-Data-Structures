...
public class Board {
    ...
    public boolean equals(Object o) {
        ...
        Board that = (Board) o;//copied from Blatt10 equals method until here

        //checks equalitiy of every attribute
        if(this.nRobots!=that.nRobots || this.robots.length!=that.robots.length || this.hwall.length!=that.hwall.length
        ||this.vwall.length!=that.vwall.length || this.ny!=that.ny || this.nx!=that.nx) return false ;

        for(int i=0;i<nRobots;i++){ // go through all the robots
            if(this.robots[i].location.equals(that.robots[i].location)==false) return false;// checks every robot's
            //location which should be equal to the objects' location , did it with the location to avoid the case
            // of having same location and differents IDs
        }
        for(int x=0;x<nx;x++){
            for(int y=0;y<ny;y++){
                if (this.vwall[x][y]!=that.vwall[x][y] || this.hwall[x][y]!=that.hwall[x][y]) return false;
                //checks every wall coordinate and if it is equal to the object's
            }
        }
        return true;
