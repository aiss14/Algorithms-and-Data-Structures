    ...
    */
    ...
    public double contrast(Coordinate p0, Coordinate p1) throws InputMismatchException {
        if (p0.x < 0 || p0.x >= sizeX() || p0.y < 0 || p0.y >= sizeY() ||
            ...
            throw new InputMismatchException("input must be >0 and <sizeY/X");
        }//copied from Pictureimage
        return Math.abs(field[p1.x][p1.y]-field[p0.x][p0.y]);
        // TODO
    }
    ...
    public void removeVPath(int[] path) {
        int [][] newmatrix =new int[sizeX()-1][sizeY()];
        for(int row=0;row<sizeY();row++){
          int c=0;// counter to go through elements of each row
            for(int col=0;col<sizeX()-1;col++){

                if(col!=path[row]){
                    newmatrix[col][row]=field[c][row];
                    c++;
                }

                else{
                    c++;// increment the counter in order to
                    newmatrix[col][row]=field[c][row];
                    c++;
